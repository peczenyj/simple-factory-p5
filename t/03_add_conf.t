...
subtest "should add new conf" => sub {
    ...
    throws_ok { $factory->resolve('c') } qr/instance of 'Foo' named 'c' not found/, 'should die: no conf for key c';

    $factory->add_build_conf_for(c => 3);

    my $object;
    lives_ok{ $object = $factory->resolve('c') } 'should not die, conf for key c added';
